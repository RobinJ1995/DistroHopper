...
The app icon source is `etc/distrohopper test4_1.xcf` (GIMP). All derived
assets are generated by `etc/generate_icon.py` — do not edit the PNGs by
hand; regenerate them from the XCF instead:

```
pip install cairosvg gimpformats Pillow   # one-time
python3 etc/generate_icon.py path/to/distrohopper_test4_1.xcf
```
...
| Output | Purpose |
|--------|---------|
| `app/src/main/res/drawable/ic_launcher_background.png` | Adaptive icon background (108×108, full-bleed orange swirl) |
| `app/src/main/res/drawable/ic_launcher_foreground.png` | Adaptive icon foreground (108×108, face on transparent) |
| `app/src/main/res/mipmap-anydpi-v26/ic_launcher{,_round}.xml` | Adaptive icon definitions (API 26+) |
| `app/src/main/res/mipmap-{mdpi,hdpi,xhdpi,xxhdpi,xxxhdpi}/ic_launcher.png` | Legacy launcher icons |
| `app/src/main/ic_launcher-web.png` | Web/Play Store icon (512×512) |
| `fastlane/metadata/android/en-US/images/icon.png` | Fastlane store listing icon |

If `gimpformats` cannot be installed, manually export the layers from the XCF
to the `/tmp/dh_*.png` cache paths the script documents, then run the script
without arguments.

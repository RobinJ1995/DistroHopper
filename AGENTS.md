...
The app icon source is `etc/icon_source.xcf` (GIMP). All derived assets are
generated by `etc/generate_icon.py` — do not edit the PNGs by hand;
regenerate them from the XCF instead:

```
pip install cairosvg gimpformats Pillow numpy   # one-time
python3 etc/generate_icon.py etc/icon_source.xcf
```

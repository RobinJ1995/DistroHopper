...
object WidgetGrid {
	...
	fun presets(context: Context): List<Pair<Int, Int>> {
		...
		val raw = prefs.getString(Preference.DESKTOP_GRID_PRESETS.getName(), null)
		val stored = raw?.split(',')?.mapNotNull { parseSize(it) }
		if (stored != null && stored.size == PRESET_COUNT) {
			return stored
		}
		if (raw != null) {
			Log.getInstance().w(TAG, "Discarding unreadable desktop grid presets: $raw")
		}
		...
		prefs.edit()
			...
			.apply()
		// One-time and layout-defining: worth a line, since every stored desktop
		// coordinate is written against whichever grid this picks. //
		Log.getInstance().i(TAG, "Snapshotted desktop grid presets for " +
			"${shortEdgeDp}x${longEdgeDp}dp: " + generated.joinToString(", ") { formatSize(it) })
	...
	fun size(context: Context): Pair<Int, Int> {
		val prefs = Preferences.getSharedPreferences(context)
		val raw = prefs.getString(Preference.DESKTOP_GRID_SIZE.getName(), null)
		val stored = parseSize(raw)
		if (stored != null) {
			return stored
		}
		if (raw != null) {
			Log.getInstance().w(TAG, "Discarding unreadable desktop grid size: $raw")
		}
		...
		prefs.edit()
			...
			.apply()
		Log.getInstance().i(TAG, "Snapshotted desktop grid size: " + formatSize(middle))
	...
	fun setSize(context: Context, size: Pair<Int, Int>) {
		Preferences.getSharedPreferences(context).edit()
			...
			.commit()
		Log.getInstance().i(TAG, "Desktop grid size changed to " + formatSize(size))
	}

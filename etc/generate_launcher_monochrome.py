...
recolours when "Themed icons" is on. A monochrome layer is a single-alpha
stencil: opaque pixels become the tint foreground, transparent pixels the tint
background (see `icons/IconRenderer.drawTinted`), so the mark has to survive
...
def preview(directory):
	...
	blades = [blade_polygon(i) for i in range(BLADES)]
	f = face_polygons()
	solids = blades + [f["left_eye"], f["right_eye"], f["beak"]]
	...
	for poly in blades:
		draw.polygon([(x * full, y * full) for x, y in poly], fill=(230, 130, 60, 255))
